...
└── packages/
    ...
    │       ├── agent-skills.ts       # SKILL.md discovery for the picker (global + project, nearest wins)
...
| `GET /api/agents/skills?agent=claude-code&path=/` | Skills a prompt may name: `~/.claude/skills` or `~/.codex/skills` plus `.agents/skills`, and the same folders from `path` up to the git root; a project skill shadows a global one of the same name — **bearer token required** |
...
- **Skills are listed, never expanded, by the server**: `agent-skills.ts` reads only `name` and `description` from each `SKILL.md` frontmatter and never sends a skill's body. The client turns a `/name` token into a prose instruction before launch, because both CLIs recognise slash commands only at the very start of the input and `buildThreadPrompt` prefixes the target context anyway. Discovery reads through symlinks on purpose (`npx skills` installs links) and uses real paths, because `git rev-parse` reports the root through resolved symlinks and the walk-up window compares the two
